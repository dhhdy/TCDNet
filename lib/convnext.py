...
class ConvNeXtV2(nn.Module):
    ...
    def __init__(self, in_chans=1, num_classes=1000,
        ...
        stem = nn.Sequential(
            nn.Conv2d(in_chans, dims[0], kernel_size=1, stride=1), #[modify]!!!!
...
def convnext_pico(pretrained=False, **kwargs):
    ...
    # if pretrained:
        # path = '/kaggle/input/lbunet/'
        # # path = 'E:/dataset/unet_work/'
        # ckpt = torch.load(path + 'pretrained/convnextv2_pico_1k_224_fcmae.pt')
...
def convnextv2_tiny(pretrained=False, **kwargs):
    ...
    if pretrained:
        ckpt = torch.load('/root/autodl-tmp/pretrained/convnextv2_tiny_1k_224_fcmae.pt')
        model.load_state_dict(ckpt, strict=False)
    return model
...
if __name__ == "__main__":
    m = convnextv2_femto(pretrained=False)
    summary(m.cuda(),input_size=(1,224,224))
